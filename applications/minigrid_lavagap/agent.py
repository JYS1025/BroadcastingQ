...
class LavaGapStructuralSBQ(SBQAgent):
    """Broadcast locally over agent pose while keeping goal/gap context fixed."""
    ...
    def get_neighborhood(self, state: int) -> np.ndarray:
        ...
        for row in range(self.observation_space.nvec[0]):
            for col in range(self.observation_space.nvec[1]):
                ...
                for direction in range(self.observation_space.nvec[2]):
                    dir_dist = 0 if direction == int(center[2]) else 1
                    if pos_dist + dir_dist <= radius:
                        obs = np.concatenate(
