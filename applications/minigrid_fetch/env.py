...
AGENT_REGISTRY["minigrid_fetch_sbq"] = "applications.minigrid_fetch.agent:MiniGridFetchSBQ"
AGENT_REGISTRY[
    "minigrid_fetch_q_learning_tiebreak"
] = "applications.minigrid_fetch.agent:MiniGridFetchTieBreakQLearning"


class MiniGridFetchEnv(BaseEnv):
    """Symbolic wrapper for MiniGrid Fetch tasks."""
    ...
    def __init__(self, config: dict) -> None:
        ...
        try:
            ...
            raise ImportError(
                "MiniGrid Fetch requires minigrid. Use the bcrl conda env or install minigrid."
            ) from exc
        ...
        env_id = self.env_config.get("minigrid_env_id", "MiniGrid-Fetch-5x5-N2-v0")
        ...
        self.max_objects = int(self.observation_config.get("max_object_slots", 3))
        ...
        default_subset = ["left", "right", "forward", "pickup"]
    ...
    def render(self, mode: str = "rgb_array") -> np.ndarray:
        if mode != "rgb_array":
            raise ValueError("MiniGridFetchEnv supports only mode='rgb_array'")
    ...
    def _convert_obs(self, raw_obs: dict[str, Any]) -> np.ndarray:
        ...
        objects = self._scan_objects(target_type=target_type, target_color=target_color)
        ...
        if not self.observation_space.contains(obs):
            raise RuntimeError(f"Converted MiniGrid Fetch observation {obs} is outside {self.observation_space}")
        return obs

    def _scan_objects(self, target_type: int, target_color: int) -> list[dict[str, int]]:
        ...
        rows.sort(key=lambda item: (-item["target"], item["type"], item["color"], item["row"], item["col"]))
    ...
    def _target_from_mission(self, mission: str) -> tuple[int, int]:
        ...
        match = re.search(
            r"^(?:get|go get|fetch|go fetch|you must fetch) a (?P<color>\w+) (?P<type>\w+)$",
            mission,
        )
        if match:
        ...
        if len(tokens) >= 2:
            color, obj_type = tokens[-2], tokens[-1]
            return self._type_idx(obj_type), self._color_idx(color)
    ...
    def _symbolic_info(self, obs: np.ndarray) -> dict:
        ...
        return {
            ...
            "target": (values["target_type"], values["target_color"]),
        }
