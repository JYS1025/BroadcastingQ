...
class DangerMazeStructuralSBQ(SBQAgent):
    ...
    def get_neighborhood(self, state: int):
        ...
        if len(center) >= 4:
            by_layout = getattr(self.observation_space, "valid_positions_by_layout", None)
            if by_layout is not None:
                valid_positions = by_layout.get(int(center[3]), valid_positions)
